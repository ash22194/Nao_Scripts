...
extern Vector3d target(800,800,1.5708);
...
void compute_neighbours(Vector3i state, std::vector<Vector3i>& neighbours, std::vector<double>& h, 
    std::map<Vector3i,bool,cmpV3i >& obstacles, std::vector<Vector3d>& moves)
  {
    // double PI;
    double f_t_c = 1;
    double r_c = 1;
            ...
            for (int j = -1; j < 2; j++)
            {
                ...
                {   
                    ...
                    else
                    {
                        ...
                        {
                            cost = s_t_c;
                            if (i*sin(theta) - j*cos(theta) > 0)
                            {
                                moves.push_back(Vector3d(0,-sqrt(2),0));
                            }
                            if (i*sin(theta) - j*cos(theta) < 0)
                            {
                                moves.push_back(Vector3d(0,sqrt(2),0));
                        ...
                        {
                            cost = f_t_c;
                            moves.push_back(Vector3d(sqrt(2),0,0));
                        }
                        else if (i*cos(theta) + j*sin(theta) < 0)
                        {
                            cost = b_t_c;
                ...
                if (!((x_ < -f_s(0)/2) || (x_ > f_s(0)/2) || (y_ < -f_s(1)/2) || (y_ > f_s(1)/2) || (blocked)))
                {
                    ...
                    else
                    {
                        ...
                        if (std::abs(i*cos(theta) + j*sin(theta)) < 0.1)
                        {
                            ...
                            if (i*sin(theta) - j*cos(theta) > 0)
                            {
                                moves.push_back(Vector3d(0,-1,0));
                            }
                            if (i*sin(theta) - j*cos(theta) < 0)
                            {
                                moves.push_back(Vector3d(0,1,0));
                        ...
                        {
                            cost = f_t_c;
                            moves.push_back(Vector3d(1,0,0));
                        }
    ...
    moves.push_back(Vector3d(0,0,1));
...
bool plan(std::vector<Vector3d>& path, std::vector<Vector3d>& action)
{
    ...
    */
    ...
    std::map<Vector3i, Vector3i, cmpV3i > tree,parent;
    std::map<Vector3i, Vector3d, cmpV3i > moves;
    ...
    {
        count_++;
        double g_next = g[next];
        double f = g_next + heuristic(next);
        closed_states[next] = f;
        std::map<Vector3i, double, cmpV3i >::iterator closed_states_it = closed_states.find(next);
        // myfile<<"Next : \n";
        // myfile<<next(0)<<","<<next(1)<<","<<next(2)<<","<<" f : "<<((closed_states_it)->second)<<", g : "<<g_next<<", h : "<<heuristic(next)<<"\n";

        std::vector<Vector3i> neighbours;
        std::vector<double> one_step_cost;
        std::vector<Vector3d> step;
        compute_neighbours(next,neighbours,one_step_cost,obstacles,step);
        ...
        for (int i = 0; i < neighbours.size(); i++)
        {
            ...
            {
                double g_i = g_next + one_step_cost[i];
                double h_i = heuristic(neighbours[i]);
                std::map<Vector3i, double, cmpV3i >::iterator g_it = g.find(neighbours[i]);
                if (g_it == g.end())
                {
                    g[neighbours[i]] = g_i;
                    (open_states[h_i + g_i])[neighbours[i]] = true;
                    tree[neighbours[i]] = next;
                    moves[neighbours[i]] = step[i];
                }
                else if (g_it->second > g_i)
                {
                    g_it->second = g_i;
                    (open_states[h_i + g_i])[neighbours[i]] = true;
                    tree[neighbours[i]] = next;   
        ...
        {   
            ...
            parent[next] = tree.find(next)->second;
        }
    ...
    // {
    //     tree_file<<"F : "<<(g.find(tree_it->first)->second + heuristic(tree_it->first))<<" Node : "<<tree_it->first.x()<<","<<tree_it->first.y()<<","<<tree_it->first.z()<<
    ...
    {   std::cout<<next(0)<<","<<next(1)<<","<<next(2)<<std::endl;
        while(next!=start_grid)
        {
            ...
            Vector3d act(moves[next].x()*g_s(0),moves[next].y()*g_s(1),moves[next].z()*g_s(2));
            Vector3i check = next;
            check(2) = 0;
